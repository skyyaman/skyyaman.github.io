...
async function getFinalVideo(flag, parseUrls, url) {
    ...
    for (const parseUrl of parseUrls) {
        ...
        const content = await request(playUrl, null, 10000); // 10绉掕姹傦紝鑳芥洿濂借繃婊ebjx
        let tryJson = null;
        try {
...
function jsonParse(input, json) {
    try {
        // 澶勭悊瑙ｆ瀽鎺ュ彛杩斿洖鐨勬姤鏂囷紝濡傛灉杩斿洖鐨勬姤鏂囦腑鍖呭惈header淇℃伅锛屽氨鍔犲埌杩斿洖鍊间腑
        let jsonPlayData = JSON.parse(json);
        // 澶勭悊293鐨勮В鏋愮粨鏋渦rl鍦╠ata瀛楁鐨勮В鏋�
        ...
        for (let key in typeExtend) {
            if (key === "class" || key === "area" || key === "lang" || key === "year") {
                try {
                    ...
                    str += "绛涢€�" + key + "+鍏ㄩ儴=+" + typeExtend[key].replace(/,/g, "+") + "\n";
                } catch (e) { }
            }
        }
    }
